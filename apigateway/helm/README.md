...
Please also check the 11.x example here: [example-11](../examples/apigw-11/README.md)
...
API Gateway supports automated post-initialization configuration through Kubernetes Jobs. This feature allows you to:

- **Automate Keystore Management**: Upload and configure custom keystores (JKS, PKCS12) after deployment
- **Configure HTTPS Ports**: Create and enable HTTPS ports programmatically using the API Gateway Admin REST API
- **Update Existing Ports**: Modify existing port configurations (e.g., change keystores, aliases, clientAuth settings)
- **Custom Configuration Tasks**: Execute any post-deployment configuration via shell scripts

### Configuration

Use the `postInitConfigMaps` and `postInitJobs` values to define your automation:
...
**Note**: Jobs run without Helm hooks (use `annotations: {}`) to avoid blocking the Helm installation. Jobs will retry automatically on failure using `restartPolicy: OnFailure`.

### Example Use Cases

- **Keystore Configuration**: Automatically upload keystores generated by cert-manager to API Gateway
- **HTTPS Port Setup**: Create runtime HTTPS ports (5556) with custom certificates
- **Port Updates**: Switch Admin HTTPS port (5543) from default IS certificate to custom certificates
- **API Configuration**: Use API Gateway Admin REST API for any configuration tasks

See the [e2e-security example](../examples/e2e-security/README.md) for a complete implementation with automated keystore and HTTPS port configuration.
...
| [e2e-security](../examples/e2e-security/README.md) | End-to-end TLS security with cert-manager, HTTPS for all components (Elasticsearch, Kibana, API Gateway UI/Admin/Runtime), backend HTTPS verification at ingress, automated keystore and port configuration via post-init jobs |
...
| apigw.runtimePort | int | `5556` | The API Gateway runtime port for API invocations. By default API Gateway images do not have this port setup. You can use `postInitJobs` to automatically configure this port after deployment. See e2e-security example for automated HTTPS port configuration. |
...
| elasticsearch.defaultNodeSet | object | `{"annotations":{},"count":1,"extraCmdPluginInstaller":"","extraConfig":{},"extraInitContainers":{},"installMapperSizePlugin":true,"memoryMapping":false,"setMaxMapCount":true}` | Default Node Set |
...
| postInitConfigMaps | list | `[]` | List of ConfigMaps to create for post-initialization scripts. Each ConfigMap can contain shell scripts used by post-init jobs. Example: `- name: post-init-scripts` with `data:` containing script definitions. See e2e-security example for usage with keystore and port configuration automation. |
| postInitJobs | list | `[]` | List of post-initialization jobs to run after API Gateway deployment. Jobs can be used to automate configuration tasks like creating keystores, configuring HTTPS ports, or other setup tasks. Supports annotations for ordering (use empty `annotations: {}` to run jobs without Helm hooks). Each job supports: `name`, `annotations`, `image`, `command`, `env`, `volumeMounts`, `volumes`, and `restartPolicy`. See e2e-security example for automated keystore and HTTPS port configuration. |
