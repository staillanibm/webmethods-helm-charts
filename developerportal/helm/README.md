...
| devportal.livenessProbe | object | `{"failureThreshold":3,"initialDelaySeconds":30,"periodSeconds":10,"successThreshold":1,"tcpSocket":{"port":"ui-http"},"timeoutSeconds":1}` | Liveness probe configuration |
| devportal.port | int | `8083` | HTTP listening port. Use on wM version 10.15 (default): `8083`, 11.0: `8080`  |
| devportal.readinessProbe | object | `{"failureThreshold":3,"initialDelaySeconds":30,"periodSeconds":10,"successThreshold":1,"tcpSocket":{"port":"ui-http"},"timeoutSeconds":1}` | Readiness probe configuration |
| devportal.startupProbe | object | `{"failureThreshold":30,"periodSeconds":10,"successThreshold":1,"tcpSocket":{"port":"ui-http"},"timeoutSeconds":1}` | Startup probe configuration |
| devportal.tls | object | `{"apigw":{"enabled":false,"truststorePasswordKey":"password","truststorePasswordSecret":"","truststorePath":"/certs/apigw/truststore.jks","truststoreSecret":""},"elastic":{"enabled":false,"truststorePasswordKey":"password","truststorePasswordSecret":"","truststorePath":"/certs/es/truststore.jks","truststoreSecret":""},"ui":{"enabled":false,"keyAlias":"devportal-ui","keyPasswordKey":"password","keyPasswordSecret":"","keystorePasswordKey":"password","keystorePasswordSecret":"","keystorePath":"/certs/ui/keystore.jks","keystoreSecret":"","port":8443}}` | TLS Configuration |
| devportal.tls.apigw | object | `{"enabled":false,"truststorePasswordKey":"password","truststorePasswordSecret":"","truststorePath":"/certs/apigw/truststore.jks","truststoreSecret":""}` | API Gateway TLS Configuration |
| devportal.tls.apigw.enabled | bool | `false` | Enable API Gateway TLS connectivity |
| devportal.tls.apigw.truststorePasswordKey | string | `"password"` | Key in the truststore password secret |
| devportal.tls.apigw.truststorePasswordSecret | string | `""` | Secret name containing the truststore password |
| devportal.tls.apigw.truststorePath | string | `"/certs/apigw/truststore.jks"` | Path where API Gateway truststore will be mounted in the container |
| devportal.tls.apigw.truststoreSecret | string | `""` | Secret name containing the API Gateway JKS truststore |
| devportal.tls.elastic | object | `{"enabled":false,"truststorePasswordKey":"password","truststorePasswordSecret":"","truststorePath":"/certs/es/truststore.jks","truststoreSecret":""}` | Elasticsearch TLS Configuration |
| devportal.tls.elastic.enabled | bool | `false` | Enable TLS for Elasticsearch connectivity |
| devportal.tls.elastic.truststorePasswordKey | string | `"password"` | Key in the truststore password secret |
| devportal.tls.elastic.truststorePasswordSecret | string | `""` | Secret name containing the truststore password Example: "cert-secret" |
| devportal.tls.elastic.truststorePath | string | `"/certs/es/truststore.jks"` | Path where truststore will be mounted in the container |
| devportal.tls.elastic.truststoreSecret | string | `""` | Secret name containing the JKS truststore (generated by cert-manager) This should be the same secret name as the Elasticsearch certificate The secret must contain a 'truststore.jks' key Example: "devportal-developerportal-es-tls-secret" |
| devportal.tls.ui | object | `{"enabled":false,"keyAlias":"devportal-ui","keyPasswordKey":"password","keyPasswordSecret":"","keystorePasswordKey":"password","keystorePasswordSecret":"","keystorePath":"/certs/ui/keystore.jks","keystoreSecret":"","port":8443}` | UI Server HTTPS Configuration |
| devportal.tls.ui.enabled | bool | `false` | Enable HTTPS for the UI server |
| devportal.tls.ui.keyAlias | string | `"devportal-ui"` | Alias of the certificate in the keystore |
| devportal.tls.ui.keyPasswordKey | string | `"password"` | Key in the key password secret |
| devportal.tls.ui.keyPasswordSecret | string | `""` | Secret name containing the key password (if different from keystore password) |
| devportal.tls.ui.keystorePasswordKey | string | `"password"` | Key in the keystore password secret |
| devportal.tls.ui.keystorePasswordSecret | string | `""` | Secret name containing the keystore password |
| devportal.tls.ui.keystorePath | string | `"/certs/ui/keystore.jks"` | Path where keystore will be mounted in the container |
| devportal.tls.ui.keystoreSecret | string | `""` | Secret name containing the JKS keystore for UI server certificate |
| devportal.tls.ui.port | int | `8443` | HTTPS port (default 8443) |
...
| extraEnvs | list | `[]` | Extra environment properties to be passed on to the container Example: extraEnvs:   - name: MY_ENV_VAR     value: "my-value"   - name: SECRET_ENV_VAR     valueFrom:       secretKeyRef:         name: my-secret         key: my-key |
...
| image.tag | string | `"10.15"` | The image tag of the apigateway image default this will be the latest version. For realworld scenarios IBM recommends to use a specific version to not accidently change production versions with newer images. |
